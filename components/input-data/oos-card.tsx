    ...
    <Card className="border-0 shadow-sm rounded-xl">
      <CardHeader className="pb-4">
        ...
        <p className="text-xs text-muted-foreground">
          Opsional. Tambahkan hanya jika ada item OOS yang ingin dilaporkan.
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="space-y-4">
          {fields.length === 0 ? (
            <div className="rounded-lg border border-dashed border-muted-foreground/30 bg-muted/20 px-4 py-5 text-sm text-muted-foreground">
              Belum ada item OOS. Klik tombol di bawah jika ingin menambahkan.
            </div>
          ) : (
                    ...
                    <div className="flex-1 space-y-1">
                      <Input
                        ...
                        {...register(`itemOos.${index}.name` as const, {
                          onChange: (e) => {
                            e.target.value = alphaNumericSpaceOnly(
                              e.target.value
                            );
                          }
                        })}
                      ...
                      {errors.itemOos?.[index]?.name && (
                        <p className="text-xs text-red-500 font-medium">
                          {errors.itemOos[index]?.name?.message}
                        </p>
                      )}
                    </div>
                    <Button
